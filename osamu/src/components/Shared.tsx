...
export const Sidebar = ({ activeView, setView, user }: { activeView: View; setView: (v: View) => void; user: UserType }) => {
  const navItems = [
  ...
  return (
    <aside className="hidden md:flex flex-col h-screen py-8 px-6 space-y-8 glass-nav h-full w-64 fixed left-0 top-0 shadow-[40px_0px_40px_-10px_rgba(6,13,32,0.5)] z-50">
      <div className="flex items-center gap-3 px-2">
        ...
        <div className="flex flex-col">
          <span className="text-xl font-extrabold tracking-tighter text-primary">Osamu</span>
      ...
      <div className="mt-auto p-4 rounded-xl bg-surface-container-low flex items-center gap-3">
        <img 
          src={user.avatar} 
          alt={user.name} 
    ...
    <header className="sticky top-0 z-40 glass-nav flex justify-between items-center w-full px-8 py-4 transition-colors duration-300 border-b border-surface-container-high">
      <div className="flex items-center gap-6 flex-1">
        <div className="relative w-full max-w-md group">
          ...
          <input 
            className="w-full bg-surface-container-lowest border-none rounded-lg pl-10 pr-4 py-2 text-sm focus:ring-0 focus:bg-surface-container-high transition-all"
  ...
  return (
    <div className="flex items-end gap-1.5 h-12">
      {Array.from({ length: count }).map((_, i) => {
        // Vary height randomly or following a pattern
