          ...
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex gap-4 items-start">
                ...
                <div>
                  <p className="text-sm font-bold text-on-surface">12 Essays Uploaded</p>
                  <p className="text-xs text-outline">Modernism Seminar • 14 mins ago</p>
                </div>
              </div>
            ))}
          </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="space-y-4">
            <div className="p-6 bg-surface-container-highest/50 rounded-xl border border-outline-variant/20 flex items-center justify-between group hover:border-primary/30 transition-all">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-on-surface">Course_Rubric_V2.pdf</p>
                  <p className="text-xs text-outline">Reference Document • 2.4 MB</p>
                </div>
              </div>
      ...
      <div className="md:col-span-5 bg-surface-container rounded-2xl p-8 shadow-2xl border border-surface-container-high flex flex-col h-full">
        ...
        <textarea 
          ...
          placeholder="e.g., 'Ensure citations follow Harvard style strictly, and subtract 2 points for every minor spelling error in technical terms...'"
        />
  ...
  return (
    <div className="bg-surface-container-low rounded-2xl border border-surface-container-high overflow-hidden shadow-2xl">
      <div className="p-8 border-b border-outline-variant/10 flex justify-between items-center">
        ...
        <div className="flex gap-4">
          <div className="relative">
            <input className="bg-surface-container px-4 py-2 pl-10 rounded-lg text-xs placeholder:text-outline border-none focus:ring-1 focus:ring-primary/20" placeholder="Search students..." />
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-outline-variant/10">
            {[
              { name: 'Sarah Jenkins', task: 'Seminar Essay', score: 92, date: 'Oct 14, 2023' },
              { name: 'Marcus Wong', task: 'Geometry Lab', score: 74, date: 'Oct 12, 2023' },
              { name: 'Elena Petrova', task: 'Physics Quiz', score: 88, date: 'Oct 11, 2023' },
              { name: 'Samuel Kim', task: 'Design Report', score: 95, date: 'Oct 10, 2023' },
            ].map((row, i) => (
              <tr key={i} className="group hover:bg-surface-container-high/30 transition-colors cursor-pointer">
                <td className="px-8 py-6">
                  <div className="flex items-center gap-3">
                    <img className="w-8 h-8 rounded-full border border-outline-variant/20" src={`https://picsum.photos/seed/hist${i}/80/80`} alt="" referrerPolicy="no-referrer" />
      ...
      <div className="bg-surface-container/30 px-8 py-4 flex justify-between items-center">
        <span className="text-xs text-outline font-bold">Page 1 of 12</span>
        <div className="flex gap-2">
          ...
          <button className="w-8 h-8 flex items-center justify-center rounded bg-surface-container-low border border-outline-variant/10 text-outline active:scale-90 transition-transform">
            <ChevronDown className="w-4 h-4 -rotate-90" />
          </button>
        </div>
      </div>
