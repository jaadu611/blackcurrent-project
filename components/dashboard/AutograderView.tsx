...
export const AutograderView = ({ user }: { user: any }) => {
  ...
  const handleRubricSave = async () => {
    if (config.topic && config.date && config.files.length > 0) {
        ...
        <div className="flex bg-white p-1.5 rounded-3xl border border-black/5 shadow-xl shadow-black/5">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-3 px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === tab.id
              ...
              <div className="bg-white rounded-[40px] p-10 border border-black/5 shadow-2xl shadow-black/5 space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  <div
                    ...
                    className={`border-2 border-dashed rounded-[32px] p-12 flex flex-col items-center justify-center gap-4 transition-all cursor-pointer group ${config.files.length > 9 ? 'border-red-500/20 bg-red-50/10' : 'border-black/10 bg-[#F8F9FA] hover:bg-black/5 hover:border-black/20'
                      }`}
                  >
