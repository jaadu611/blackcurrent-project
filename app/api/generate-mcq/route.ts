...
export async function POST(req: Request) {
  try {
    // Direct Push to ESP32
    const STATION_IP = process.env.STATION_IP || "10.30.233.98";
    const espUrl = `http://${STATION_IP}/api/load_questions`;
    const notebookTitle = `Debug MCQ Session - ${Date.now()}`;

    console.log(`[BYPASS-PUSH] Actively pushing debug quiz to ESP at ${espUrl}`);

    // Push actively to ESP
    fetch(espUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(debugQuestions),
    }).then(res => {
      if (res.ok) console.log("[BYPASS-PUSH] Successfully synced to ESP32");
      else console.error("[BYPASS-PUSH] ESP32 returned error:", res.status);
    }).catch(err => {
      console.warn("[BYPASS-PUSH] Could not reach ESP32 station:", err.message);
    });

    // Return result to UI
    return NextResponse.json({ text: JSON.stringify(debugQuestions) });
