...
export async function GET() {
    ...
    const STATION_IP = process.env.STATION_IP || "192.168.1.15";
    const espUrl = `http://${STATION_IP}/api/load_questions`;

    console.log(`[DEBUG-PUSH] Actively pushing debug quiz to ESP at ${espUrl}`);
    ...
    try {
        ...
        if (resp.ok) {
            return NextResponse.json({ message: "Successfully pushed debug quiz to ESP32" });
        } else {
            return NextResponse.json({ error: "ESP32 rejected the push", status: resp.status }, { status: 502 });
        }
    } catch (err: any) {
        return NextResponse.json({ error: "Failed to reach ESP32", details: err.message }, { status: 504 });
    }
}
