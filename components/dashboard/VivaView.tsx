...
interface Question {
  type: string;
  question: string;
  options?: { text: string; isCorrect: boolean; followUp?: any }[];
  answer?: any;
  followUp?: any;
}
...
export const VivaView = ({ user }: { user: any }) => {
  const [activeTab, setActiveTab] = useState<"creator" | "autograder" | "questions" | "results">("creator");
  ...
  const [resultJsonInput, setResultJsonInput] = useState("");
  ...
  const handleProcessJson = () => {
    try {
      ...
      alert("Invalid Question JSON format.");
    }
  };

  const handleProcessResultJson = () => {
    try {
      const parsed = JSON.parse(resultJsonInput);
      if (parsed.totalScore !== undefined) {
        setAiResult(parsed);
        setResultJsonInput("");
      }
    } catch (e) {
      alert("Invalid Result JSON format.");
    }
  };
    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'questions' && (
          ...
          >
            {/* JSON Injector Area */}
            <div className="bg-white rounded-[32px] p-8 border border-black/5 shadow-xl shadow-black/[0.02] space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-black rounded-lg flex items-center justify-center text-white">
                    <Code2 size={16} />
                  </div>
                  <span className="text-sm font-black tracking-tight">Inject Quiz JSON</span>
                </div>
                {jsonInput && (
                  <button
              ...
              <textarea
                placeholder="Paste question pool JSON here..."
                className="w-full h-32 bg-black/[0.02] border border-black/5 rounded-2xl p-4 text-xs font-mono focus:outline-none focus:ring-1 focus:ring-black/10 transition-all resize-none"
                value={jsonInput}
                  ...
                  <div key={qIdx} className="bg-white p-8 rounded-[32px] border border-black/5 shadow-xl shadow-black/[0.02] flex flex-col gap-6 group hover:-translate-y-1 transition-all duration-300">
                    ...
                    <div className="space-y-2">
                      {q.options?.map((opt, oIdx) => (
                        <div key={oIdx} className="space-y-4">
                          <div className={`text-xs p-3 rounded-xl border flex items-center justify-between font-bold ${opt.isCorrect ? 'bg-black text-white border-black shadow-lg shadow-black/10' : 'bg-[#F8F9FA] border-black/5 text-black/40'}`}>
                            <span>{opt.text}</span>
                            {opt.isCorrect && <CheckCircle2 size={12} />}
                          </div>
                          {opt.followUp && (
                            <div className="ml-4 pl-4 border-l-2 border-black/5 space-y-2">
                              <span className="text-[8px] font-black uppercase tracking-widest text-black/20">Follow-up for {opt.text.split(')')[0]})</span>
                              <p className="text-[10px] font-bold text-black/60 italic leading-snug">"{opt.followUp.question}"</p>
                              {opt.followUp.type === 'mcq' && (
                                <div className="flex flex-wrap gap-1">
                                  {opt.followUp.options.map((fo: string) => (
                                    <span key={fo} className="text-[8px] px-2 py-0.5 bg-black/5 rounded-md font-bold">{fo}</span>
                                  ))}
                                </div>
                              )}
                            </div>
                    ...
                    {q.followUp && (
                      <div className="mt-auto pt-6 border-t border-black/5 text-xs">
                        <span className="text-[9px] font-black uppercase tracking-widest text-black/20 block mb-2">Primary Adaptive Context</span>
                        <p className="font-bold text-black/60 italic">"{q.followUp.question}"</p>
                        {q.followUp.type === 'numeric' && <span className="text-[9px] font-black opacity-40">Target: {q.followUp.answer}</span>}
                      </div>
                    )}
                  </div>
                ))
        ...
        {activeTab === 'results' && (
          <motion.div
            ...
            className="space-y-8"
          >
            {/* Result JSON Injector */}
            <div className="bg-white rounded-[40px] p-12 border border-black/5 shadow-2xl shadow-black/5 space-y-8">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-black rounded-2xl flex items-center justify-center text-white">
                  <ClipboardCheck size={28} />
                </div>
                <div>
                  <h3 className="text-2xl font-black tracking-tighter">Import Assessment Result</h3>
                  <p className="text-xs font-bold text-black/40 uppercase tracking-widest mt-1">Paste NotebookLM evaluation JSON</p>
                </div>
              </div>

              <div className="space-y-4">
                <textarea
                  placeholder='Paste { "totalScore": ... } here'
                  className="w-full h-40 bg-black/[0.02] border border-black/5 rounded-[32px] p-8 text-sm font-mono focus:outline-none focus:ring-4 focus:ring-black/5 transition-all resize-none"
                  value={resultJsonInput}
                  onChange={(e) => setResultJsonInput(e.target.value)}
                />
                <button
                  onClick={handleProcessResultJson}
                  disabled={!resultJsonInput}
                  className="w-full py-5 rounded-3xl bg-black text-white text-xs font-black uppercase tracking-widest hover:scale-[1.01] active:scale-95 transition-all shadow-xl shadow-black/10 disabled:opacity-20"
                >
            ...
            {aiResult && (
              <motion.div
                ...
                className="bg-white rounded-[40px] p-12 border border-black/5 shadow-2xl shadow-black/5 relative overflow-hidden group"
              >
                <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between mb-12 pb-12 border-b border-black/5 gap-10">
                  <div className="flex items-center gap-8">
                    ...
                    <div>
                      <h4 className="text-4xl font-black tracking-tighter">Global Assessment Index</h4>
                      <span className="text-[10px] font-black uppercase tracking-widest text-black/20 mt-1 block">Multi-Point Cognitive Evaluation</span>
                    </div>
                  </div>
                  <div className="bg-[#F8F9FA] p-8 rounded-[32px] border border-black/5 text-center min-w-[200px]">
                    <span className="text-xs font-black text-black/20 uppercase tracking-widest block mb-2">Composite Score</span>
                    <h3 className="text-7xl font-black tracking-tighter leading-none tabular-nums text-black">{aiResult.totalScore}%</h3>
                  </div>
                </div>
                ...
                <div className="space-y-12">
                  <div className="space-y-6">
                    <label className="text-[10px] font-black uppercase tracking-widest text-black/20 flex items-center gap-3">
                      <MessageSquare size={16} /> AI Qualitative Analysis
                    </label>
                    <div className="p-10 bg-[#F8F9FA] rounded-[32px] relative italic border border-black/5 shadow-inner">
                      ...
                      <p className="text-xl font-bold tracking-tight text-black relative z-10">{aiResult.overallFeedback}</p>
                    </div>
                  </div>

                  <div className="space-y-6">
                    <label className="text-[10px] font-black uppercase tracking-widest text-black/20 flex items-center gap-3">
                      <History size={16} /> Granular Question Breakdown
                    </label>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {aiResult.questionResults.map((res) => (
                        <div key={res.index} className={`p-8 rounded-[32px] border flex gap-6 ${res.isCorrect ? 'bg-black text-white border-black shadow-xl shadow-black/10' : 'bg-[#F8F9FA] border-black/5'}`}>
                          <div className={`w-12 h-12 shrink-0 rounded-2xl flex items-center justify-center text-lg font-black ${res.isCorrect ? 'bg-white text-black' : 'bg-black text-white'}`}>
                            {res.index}
                          </div>
                          <div className="space-y-2">
                            <div className="flex items-center gap-2">
                              {res.isCorrect ? <CheckCircle2 size={16} /> : <AlertCircle size={16} className="text-red-500" />}
                              <span className="text-[10px] font-black uppercase tracking-widest opacity-40">{res.isCorrect ? 'Validated' : 'Discrepancy Found'}</span>
                            </div>
                            <p className={`text-sm font-bold tracking-tight leading-relaxed ${res.isCorrect ? 'text-white/80' : 'text-black/60'}`}>{res.feedback}</p>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {!aiResult && submissions.filter(s => s.aiFeedback).length > 0 && (
              <div className="space-y-8">
                <label className="text-[10px] font-black uppercase tracking-[0.3em] text-black/20 block text-center">Previous Database Records</label>
                {submissions.filter(s => s.aiFeedback).map((sub, sIdx) => (
                  <div key={sIdx} className="bg-white/40 grayscale opacity-40 rounded-[40px] p-12 border border-black/5 space-y-8">
                    <div className="flex justify-between items-center">
                      <h4 className="text-2xl font-black">{sub.studentId?.name || sub.rollNumber}</h4>
                      <span className="text-4xl font-black">{sub.aiScore}%</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.98, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98, y: 20 }} className="relative w-full max-w-6xl h-full bg-white rounded-[40px] border border-black/5 shadow-[0_64px_128px_-32px_rgba(0,0,0,0.1)] flex flex-col overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-12 space-y-12 scollbar-hide">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  <ModalStat label="Final Grade" value={`${selectedSubmission.aiScore || selectedSubmission.score}%`} icon={<Trophy size={24} />} />
