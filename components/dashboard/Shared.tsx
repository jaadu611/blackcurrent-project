...
export const Sidebar = ({ 
  ...
  return (
    <>
      ...
      <aside className={`
        flex flex-col h-screen py-8 px-6 space-y-8 glass-nav w-64 fixed left-0 top-0 shadow-[40px_0px_40px_-10px_rgba(6,13,32,0.5)] z-50 transition-transform duration-300
      ...
      `}>
        <div className="flex items-center justify-between gap-3 px-2">
          ...
          <button onClick={() => setMobileOpen?.(false)} className="md:hidden p-2 text-outline">
            <X className="w-5 h-5" />
          </button>
        </div>
  ...
  return (
    <header className="sticky top-0 z-40 glass-nav flex justify-between items-center w-full px-8 py-4 transition-colors duration-300 border-b border-surface-container-high">
      <div className="flex items-center gap-4 md:gap-6 flex-1">
        <button onClick={onMenuClick} className="md:hidden p-2 text-on-surface hover:bg-surface-container rounded-lg transition-colors">
